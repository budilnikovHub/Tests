#!/bin/bash
# profile_miner_v3.sh
# Автоматически чинит perf для ядра HiveOS/Ubuntu и выполняет CPU/GPU профилирование (без --timeout)
...
if command -v perf >/dev/null 2>&1; then
  if command -v timeout >/dev/null 2>&1; then
    echo "[Perf] Используем timeout для ограничения $DURATION секунд"
    sudo timeout "${DURATION}s" perf record -F 200 -g -- $BINARY $ARGS || true
  else
    echo "[Perf] WARNING: команда timeout не найдена, используем perl fallback"
    perl -e "alarm shift @ARGV; exec @ARGV" "$DURATION" perf record -F 200 -g -- $BINARY $ARGS || true
  fi
